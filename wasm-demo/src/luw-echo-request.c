 ...
 * Compile with:
 *
 * clang -Wall -Wextra -fno-strict-aliasing --target=wasm32-wasi --sysroot=../wasi-sysroot -Wl,--no-entry,--export=__heap_base,--export=__data_end,--export=malloc,--export=free,--stack-first,-z,stack-size=$((8*1024*1024)) -mexec-model=reactor -o libunit-wasm-demo.wasm libunit-wasm-demo.c
 *
...
#define BUF_ADD_HF(fmt, name, value) \
	...
	luw_mem_writep(ctx, " *** Welcome to WebAssembly on Unit! ***\n\n");
