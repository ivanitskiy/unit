...
RUN wget -O- https://github.com/bytecodealliance/wasmtime/releases/download/v10.0.1/wasmtime-v10.0.1-$(arch)-linux-c-api.tar.xz \
    | tar Jxfv - && \
    mkdir /usr/lib/wasmtime && \
    cp /src/wasmtime-v10.0.1-$(arch)-linux-c-api/lib/* /usr/lib/wasmtime

# Build NGINX JavaScript (njs) so that we have a feature-complete Unit
#
RUN git clone --branch 0.7.12 https://github.com/nginx/njs.git && \
    cd njs && \
...
RUN git clone https://github.com/nginx/unit.git && \
    ...
    ./configure wasm --include-path=/src/wasmtime-v10.0.1-$(arch)-linux-c-api/include \
                     --lib-path=/usr/lib/wasmtime && \
    make
