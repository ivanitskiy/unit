...
static void
exit_with_error(const char *message, wasmtime_error_t *error,
                wasm_trap_t *trap)
{
...
nxt_wasmtime_execute_hook(const nxt_wasm_ctx_t *ctx, nxt_wasm_fh_t hook)
{
    ...
    if (error != NULL || trap != NULL)
        exit_with_error("failed to call function [->hook]", error, trap);
}
    ...
    { /* DEBUG */
        ...
        for (f = wr->fields; f < f_end; f++)
            printf("== [%u/%.*s] = [%u/%u/%.*s]\n",
                   ...
                   (uint8_t *)wr + f->name_offs,
                   f->value_len, f->value_offs, f->value_len,
                   (uint8_t *)wr + f->value_offs);
    }
    error = wasmtime_func_call(rt_ctx->ctx, func, args, i, results, 1, &trap);
    if (error != NULL || trap != NULL)
        exit_with_error("failed to call function [->wasm_request_handler]",
                        error, trap);
}
...
nxt_wasmtime_get_function_exports(nxt_wasm_ctx_t *ctx)
{
    ...
    for (i = 0; i < NXT_WASM_FH_NR; i++) {
       ...
       wasmtime_linker_get(rt_ctx->linker, rt_ctx->ctx, "", 0,
                           ctx->fh[i].func_name,
...
nxt_wasmtime_init_memory(nxt_wasm_ctx_t *ctx)
{
    ...
    error = wasmtime_func_call(rt_ctx->ctx, func, args, i, results, 1, &trap);
    if (error != NULL || trap != NULL)
        exit_with_error("failed to call function [->wasm_malloc_handler]",
                        error, trap);

    wasmtime_linker_get(rt_ctx->linker, rt_ctx->ctx, "", 0, "memory",
                        strlen("memory"), &item);
...
nxt_wasmtime_init(nxt_wasm_ctx_t *ctx)
{
    ...
    nxt_wasmtime_get_function_exports(ctx);

    nxt_wasmtime_init_memory(ctx);
